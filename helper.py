...
def process_csv(path: str):
    with open(path, "r") as csvfile:
        for line in csv.reader(csvfile):
            if line[0] == 'car_model':
                continue
            printer.save_data(clean({col_names[i]: line[i] for i in range(len(col_names))}))


def process_json(path: str):
    with open(path) as jsonfile:
        for line in jsonfile:
            printer.save_data(clean(dict(json.loads(line))))


def process_xml(path: str):
    for child in etree.parse(path).getroot():
        printer.save_data(clean({x.tag: x.text for x in child}))
